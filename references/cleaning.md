...
0. **L0 个人词表硬闸（先于一切 LLM 判断）**：跑 `scripts/lint-lexicon.sh <文件>`，按 [个人词表](./cleaning/personal-lexicon.md) 黑名单逐词检查；命中必改（替换方向见词表）。白名单词（闭环、落地、抓手、赋能等用户/公司常用语）**优先级高于本文件的通用黑话规则**，禁止误杀。用户每点名一个新词，先追加进词表再改稿。正式汇报文稿（月报、自评、总结、对外文档）交付前必须跑一遍并达到 PASS。
